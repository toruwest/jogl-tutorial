...
public class NewtMouseHandleSample_KeyAdapter implements GLEventListener { //(1)// com.jogamp.newt.event.MouseListener を削除

	public static void main(String[] args) {
		new NewtMouseHandleSample_KeyAdapter();
	}

	private final List<Point2D> points;
	private int height;

	public NewtMouseHandleSample_KeyAdapter() {
		...
		//glWindow.addMouseListener(this);//削除
	...
	public void reshape(GLAutoDrawable drawable, int x, int y, int width, int height) {
		...
		//gl.glViewport(0, 0, width, height); //(5) Jogl内部で実行済みなので、不要。(APIDOCに書いてある)
...
//		gl.glOrthof(x, x + width, y + height, y, -1.0f, 1.0f); //(7)
//		gl.glOrthof(x, x + width, y + height, y, -1.0f, 1.0f); //(7)
		gl.glOrthof(x, x + width, y , y + height, -1.0f, 1.0f); //(7)
...
//					pointAdded = false;
//				}
//				System.out.printf("i:%d, x:%f, y:%f%n", i, p1.getX(), p1.getY());
//			}
